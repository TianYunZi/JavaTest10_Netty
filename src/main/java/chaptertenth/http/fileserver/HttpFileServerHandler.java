...
public class HttpFileServerHandler extends SimpleChannelInboundHandler<FullHttpRequest> {
    ...
    protected void channelRead0(ChannelHandlerContext ctx, FullHttpRequest request) throws Exception {
        ...
        try {
            accessFile = new RandomAccessFile(file, "r");//以只读方式打开
        ...
        response.headers().set(HttpHeaders.Names.CONTENT_LENGTH, Long.valueOf(fileLength));
        setContentTypeHeader(response, file);
        if (HttpHeaders.isKeepAlive(request)) {
            response.headers().set(HttpHeaders.Names.CONNECTION, HttpHeaders.Values.KEEP_ALIVE);
        }
        ctx.write(response);
        ChannelFuture sendFileFuture = ctx.write(new ChunkedFile(accessFile, 0, fileLength, 8192), ctx.newProgressivePromise());
